...
void SP_monster_turret(edict_t* self)
{
	...
	if (!st.was_key_specified("power_armor_type") && M_TURRET_POWER_ARMOR_TYPE != IT_NULL) {
		self->monsterinfo.power_armor_type = static_cast<item_id_t>(M_TURRET_POWER_ARMOR_TYPE);
		if (!st.was_key_specified("power_armor_power"))
			self->monsterinfo.power_armor_power = M_TURRET_ADDON_POWER_ARMOR(self);
	}

	// Regular armor configuration
	if (!st.was_key_specified("armor_type") && M_TURRET_INITIAL_ARMOR > 0) {
		self->monsterinfo.armor_power = M_TURRET_ADDON_ARMOR(self);
	}


	int base_health = M_TURRET_INITIAL_HEALTH;
