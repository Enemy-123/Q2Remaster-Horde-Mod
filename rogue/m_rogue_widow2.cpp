...
//  Changed end frame from FRAME_death44 to FRAME_death28 to match the 16-element array.
...
void SP_monster_widow2(edict_t* self)
{
	...
	self->monsterinfo.monster_type_id = static_cast<uint8_t>(horde::MonsterTypeID::WIDOW2);	if (g_horde->integer) {
		if (brandom())
	...
	if (!st.was_key_specified("power_armor_type") && M_WIDOW2_POWER_ARMOR_TYPE != IT_NULL) {
		self->monsterinfo.power_armor_type = static_cast<item_id_t>(M_WIDOW2_POWER_ARMOR_TYPE);
		if (!st.was_key_specified("power_armor_power"))
			self->monsterinfo.power_armor_power = M_WIDOW2_ADDON_POWER_ARMOR(self);
	}

	// Regular armor configuration
	if (!st.was_key_specified("armor_type") && M_WIDOW2_INITIAL_ARMOR > 0) {
		self->monsterinfo.armor_power = M_WIDOW2_ADDON_ARMOR(self);
	}
	...
	self->monsterinfo.monster_slots = 4; // Puedes ajustar este valor según sea necesario
