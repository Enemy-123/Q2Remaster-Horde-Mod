...
TOUCH(barrel_touch_original) (edict_t* self, edict_t* other, const trace_t& tr, bool other_touching_self) -> void
{
	float  ratio;
	vec3_t v;
...
THINK(barrel_explode_original) (edict_t* self) -> void
{
	self->takedamage = false;
...
THINK(barrel_burn_original) (edict_t* self) -> void
{
	if (level.time >= self->timestamp)
		self->think = barrel_explode_original;
...
DIE(barrel_delay_original) (edict_t* self, edict_t* inflictor, edict_t* attacker, int damage, const vec3_t& point, const mod_t& mod) -> void
{
	// allow "dead" barrels waiting to explode to still receive knockback
	if (self->think == barrel_burn_original || self->think == barrel_explode_original)
		return;
	...
	else
	{
		...
		self->think = barrel_burn_original;
		self->activator = attacker;
	}
...
THINK(barrel_think_original) (edict_t* self) -> void
{
	// the think needs to be first since later stuff may override.
	self->think = barrel_think_original;
	self->nextthink = level.time + FRAME_TIME_S;
...
THINK(barrel_start_original) (edict_t* self) -> void
{
	M_droptofloor(self);
	self->think = barrel_think_original;
	self->nextthink = level.time + FRAME_TIME_S;
...
void SP_misc_explobox(edict_t* self)
{
	...
	self->die = barrel_delay_original;
	self->takedamage = true;
	self->flags |= FL_TRAP;

	if (!self->spawnflags.has(SPAWNFLAG_EXPLOBOX_NO_MOVE))
		self->touch = barrel_touch_original;
	else
		self->flags |= FL_NO_KNOCKBACK;

	// PGM - change so barrels will think and hence, blow up
	self->think = barrel_start_original;
	self->nextthink = level.time + 20_hz;
	// PGM
