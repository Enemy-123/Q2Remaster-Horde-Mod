...
void floater_wham(edict_t* self)
{
	...
	if (!fire_hit(self, aim, damage > 0 ? damage : irandom(5, 11), -50))
		self->monsterinfo.melee_debounce_time = level.time + 3_sec;
}
...
void SP_monster_floater(edict_t* self)
{
	...
	self->monsterinfo.monster_type_id = static_cast<uint8_t>(horde::MonsterTypeID::FLOATER);	if (g_horde->integer && current_wave_level <= 18)
	{
	...
	// Power armor
	if (!st.was_key_specified("power_armor_type") && M_FLOATER_POWER_ARMOR_TYPE != IT_NULL) {
		self->monsterinfo.power_armor_type = static_cast<item_id_t>(M_FLOATER_POWER_ARMOR_TYPE);
		if (!st.was_key_specified("power_armor_power"))
			self->monsterinfo.power_armor_power = M_FLOATER_ADDON_POWER_ARMOR(self);
	}

	// Armor
	if (!st.was_key_specified("armor_type") && M_FLOATER_INITIAL_ARMOR > 0) {
		self->monsterinfo.armor_type = IT_ARMOR_COMBAT;
		if (!st.was_key_specified("armor_power"))
			self->monsterinfo.armor_power = M_FLOATER_ADDON_ARMOR(self);
	}

	if (g_horde && g_horde->integer && current_wave_level > 0) {
		self->health = M_FLOATER_ADDON_HEALTH(self);
	} else {
		self->health = static_cast<int>(M_FLOATER_INITIAL_HEALTH * st.health_multiplier);
	}
	self->gib_health = -80;
	self->mass = 300;
	self->s.scale = 0.9f;
