...
void SP_monster_gekk(edict_t* self)
{
	...
	self->monsterinfo.monster_type_id = static_cast<uint8_t>(horde::MonsterTypeID::GEKK);	if (g_horde->integer) {
		{
			if (brandom())
	...
	if (!st.was_key_specified("power_armor_type") && M_GEKK_POWER_ARMOR_TYPE != IT_NULL) {
		self->monsterinfo.power_armor_type = static_cast<item_id_t>(M_GEKK_POWER_ARMOR_TYPE);
		if (!st.was_key_specified("power_armor_power"))
			self->monsterinfo.power_armor_power = M_GEKK_ADDON_POWER_ARMOR(self);
	}

	// Regular armor configuration
	if (!st.was_key_specified("armor_type") && M_GEKK_INITIAL_ARMOR > 0) {
		self->monsterinfo.armor_power = M_GEKK_ADDON_ARMOR(self);
	}


	extern int16_t current_wave_level;
	if (g_horde && g_horde->integer && current_wave_level > 0) {
		self->health = M_GEKK_ADDON_HEALTH(self);
	} else {
		self->health = M_GEKK_INITIAL_HEALTH * st.health_multiplier;
	}
...
void SP_monster_gekkkl(edict_t* self)
{
	...
	self->monsterinfo.monster_type_id = static_cast<uint8_t>(horde::MonsterTypeID::GEKKKL);	// Call base gekk spawn
	...
	extern int16_t current_wave_level;
	if (g_horde && g_horde->integer && current_wave_level > 0) {
		self->health = M_GEKKKL_ADDON_HEALTH(self);
	} else {
		self->health = M_GEKKKL_INITIAL_HEALTH * ED_GetSpawnTemp().health_multiplier;
	}
	self->gib_health = -80;
	self->mass = 400;
