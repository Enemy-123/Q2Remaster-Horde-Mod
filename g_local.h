...
enum ent_flags_t : uint64_t {
	...
	FL_BOSS_SHORTENED = bit_v<35> // deployable's lifetime was cut by a boss; don't refresh/restore it
};
...
struct monsterinfo_t
{
	...
	vec3_t hard_separation_vector;   // [Horde] Cached full (non-lateral) un-stacking push for summoned stroggs deep-overlapping a different-team monster (transient, not saved). Lets a strogg un-pile from an enemy that lateral repulsion can't separate.
...
constexpr int32_t DEFAULT_BULLET_HSPREAD = 275;//300; //horde was 250
constexpr int32_t DEFAULT_BULLET_VSPREAD = 425; //500; //horde was 400
...
struct cached_assetindex
{
	...
	// assigned from spawn functions - uses direct gi.modelindex/soundindex/imageindex
