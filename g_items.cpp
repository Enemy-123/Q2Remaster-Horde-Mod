...
bool Pickup_Health(edict_t* ent, edict_t* other)
{
	...
	constexpr int mega_health_bonus_cap = 150;
	...
	// Cap mega health bonus amount regardless of current player health.
	if (is_mega_health && count > mega_health_bonus_cap)
		count = mega_health_bonus_cap;
...
TOUCH(Touch_Item) (edict_t* ent, edict_t* other, const trace_t& tr, bool other_touching_self) -> void
{
	...
	else if (coop->integer && P_UseCoopInstancedItems())
	{
		if (ent->item_picked_up_by[other->s.number - 1])
			return;
	}
	...
	if (taken)
	{
		...
		{
			// En deathmatch normal
			should_remove = ent->spawnflags.has(SPAWNFLAG_ITEM_DROPPED | SPAWNFLAG_ITEM_DROPPED_PLAYER);

			// Los ítems normales (no dropped) usan el sistema de respawn
			if (!should_remove)
			{
				SetRespawn(ent, 30_sec);
				return;
		...
		{
			// Single player - eliminar todos los ítems recogidos
			should_remove = true;
		}
		...
		if (item->id == IT_ITEM_SENTRYGUN || item->id == IT_ITEM_STROGGSUMM) {
			...
			for (auto player : active_players()) {
				if (player && player->client && !(player->svflags & SVF_BOT)) {
					dropped->item_picked_up_by[player->s.number - 1] = true;
				}
			}
		}
	}
...
void SetItemNames()
{
	...
	for (item_id_t i = IT_NULL; i < IT_TOTAL; i = static_cast<item_id_t>(i + 1))
	{
		...
		if (cs_index >= MAX_WHEEL_ITEMS) {
			gi.Com_Error("out of wheel indices for weapons");
			return;
		}
	...
	for (item_id_t i = IT_NULL; i < IT_TOTAL; i = static_cast<item_id_t>(i + 1))
	{
		...
		if (cs_index >= MAX_WHEEL_ITEMS) {
			gi.Com_Error("out of wheel indices for powerups");
			return;
		}
