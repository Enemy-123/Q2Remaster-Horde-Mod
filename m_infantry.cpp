...
void infantry_grenade(edict_t* self)
{
	...
	vec3_t enemy_pos = self->enemy->s.origin + (self->enemy->velocity * 0.5f); // Ajuste del tiempo de predicción para que sea más rápido
	float time_to_target = (enemy_pos - start).length() / speed;
	...
	if (dist > 200)
	{
		// Ajuste vertical más controlado para distancias largas
	...
	// Ajustar la inclinación hacia abajo según la distancia para compensar la trayectoria
	float pitch_adjust = -0.10f - (dist * 0.00010f);
	aim += up * pitch_adjust;
	aim.normalize();
	...
	float downwardAdjustment = -150.0f * gravityAdjustment / speed;
	...
	if (dist > 600)
	{
		speed += 200; // Aumentar la velocidad si el objetivo está muy lejos
	}

	// Lanzar la granada
	fire_grenade2(self, start, aim, 50, speed, 2.5_sec, 120, false);
