...
void SP_monster_parasite(edict_t* self)
{
	...
	if (!st.was_key_specified("power_armor_type") && M_PARASITE_POWER_ARMOR_TYPE != IT_NULL) {
		self->monsterinfo.power_armor_type = static_cast<item_id_t>(M_PARASITE_POWER_ARMOR_TYPE);
		if (!st.was_key_specified("power_armor_power"))
			self->monsterinfo.power_armor_power = M_PARASITE_ADDON_POWER_ARMOR(self);
	}

	// Regular armor configuration
	if (!st.was_key_specified("armor_type") && M_PARASITE_INITIAL_ARMOR > 0) {
		self->monsterinfo.armor_type = IT_ARMOR_COMBAT;
		if (!st.was_key_specified("armor_power"))
			self->monsterinfo.armor_power = M_PARASITE_ADDON_ARMOR(self);
	}
...
{
	const spawn_temp_t& st = ED_GetSpawnTemp();
    self->monsterinfo.monster_type_id = static_cast<uint8_t>(horde::MonsterTypeID::PERRO_KL);	SP_monster_parasite(self);
	self->s.skinnum = 2;
	self->yaw_speed = 65;
	//self->style = 1;
	//self->s.alpha = 0.4f;
