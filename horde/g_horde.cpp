...
static void Horde_InitLevel(const int32_t lvl) {
	// Cachear el tamaño del mapa y parámetros necesarios
	...
	// Obtener parámetros de condiciones
...
// Actualizar GetBossList para devolver el arreglo correcto
	...
	if (mapSize.isMediumMap || mapname == "rdm8" || mapname == "xdm1") {
		if (mapname == "mgu6m3" || mapname == "rboss") {
			...
			if (filteredMediumBossList.empty()) {
				for (size_t i = 0; i < BOSS_MEDIUM_SIZE; ++i) {
					if (std::strcmp(BOSS_MEDIUM[i].classname, "monster_guardian") != 0 &&
						std::strcmp(BOSS_MEDIUM[i].classname, "monster_psxguardian") != 0) {
						filteredMediumBossList.push_back(BOSS_MEDIUM[i]);
					}
				}
			}
	...
	if (mapSize.isBigMap || mapname == "test/spbox" || mapname == "q2ctf4") {
		if (mapname == "test/spbox" || mapname == "q2ctf4") {
			...
			if (filteredLargeBossList.empty()) {
				for (size_t i = 0; i < BOSS_LARGE_SIZE; ++i) {
					if (std::strcmp(BOSS_LARGE[i].classname, "monster_boss5") != 0) {
						filteredLargeBossList.push_back(BOSS_LARGE[i]);
					}
				}
			}
...
// Actualizar GetBossListSize para usar los tamaños conocidos
static size_t GetBossListSize(const MapSize& mapSize, const std::string& mapname, const boss_t* boss_list) {
	...
	if ((mapname == "mgu6m3" || mapname == "rboss") && boss_list != nullptr) {
		// Determinar el tamaño de la lista filtrada de medium
		size_t size = 0;
		for (size_t i = 0; i < BOSS_MEDIUM_SIZE; ++i) {
			if (std::strcmp(BOSS_MEDIUM[i].classname, "monster_guardian") != 0 &&
				std::strcmp(BOSS_MEDIUM[i].classname, "monster_psxguardian") != 0) {
	...
	if ((mapname == "test/spbox" || mapname == "q2ctf4") && boss_list != nullptr) {
		// Determinar el tamaño de la lista filtrada de large
		size_t size = 0;
		for (size_t i = 0; i < BOSS_LARGE_SIZE; ++i) {
			if (std::strcmp(BOSS_LARGE[i].classname, "monster_boss5") != 0) {
				size++;
	...
	// Para listas estándar
	if (boss_list == BOSS_SMALL || mapSize.isSmallMap) return BOSS_SMALL_SIZE;
	if (boss_list == BOSS_MEDIUM || mapSize.isMediumMap) return BOSS_MEDIUM_SIZE;
	if (boss_list == BOSS_LARGE || mapSize.isBigMap) return BOSS_LARGE_SIZE;
...
static void AddRecentBoss(const char* classname) {
	...
	recent_bosses.push_back(classname);
	while (recent_bosses.size() > MAX_RECENT_BOSSES) {
...
static const char* G_HordePickBOSS(const MapSize& mapSize, const std::string& mapname, int32_t waveNumber, edict_t* bossEntity) {
	...
	if (!eligible_bosses.empty()) {
		...
		const boss_t* chosen_boss = eligible_bosses[dist(mt_rand)];

		// Registrar el jefe como reciente de manera segura
...
void CleanUpSpawnPointsData() {
	...
	for (auto it = spawnPointsData.begin(); it != spawnPointsData.end(); ) {
		const auto& [spawn_point, spawn_data] = *it;
...
static const char* G_HordePickMonster(edict_t* spawn_point) {
	...
	auto spawn_it = spawnPointsData.find(spawn_point);
	if (spawn_it != spawnPointsData.end() && spawn_it->second.isTemporarilyDisabled) {
	...
	// Verificar cooldown
	auto cooldown_it = lastSpawnPointTime.find(spawn_point);
	if (cooldown_it != lastSpawnPointTime.end() && level.time < cooldown_it->second + SPAWN_POINT_COOLDOWN) {
	...
	static std::vector<const weighted_item_t*> eligible_monsters;
	eligible_monsters.clear();
	eligible_monsters.reserve(std::size(monsters));

	double total_weight = 0.0;

	// Precalcular condiciones basadas en el nivel y estado de monstruos voladores
	const bool allowFlying = g_horde_local.level >= WAVE_TO_ALLOW_FLYING;
	const bool isFlyingMode = flying_monsters_mode;
	...
	for (const auto& item : monsters) {
		...
		if ((isFlyingMode && !isFlyingMonster) ||
			(spawn_point->style == 1 && !isFlyingMonster) ||
			(!isFlyingMode && isFlyingMonster && spawn_point->style != 1 && flyingSpawns > 0) ||
			(item.min_level > g_horde_local.level || (item.max_level != -1 && item.max_level < g_horde_local.level)) ||
			(isFlyingMonster && !allowFlying)) {
			continue;
		}
	...
	for (const auto& monster : eligible_monsters) {
		cumulative_weight += monster->weight * (IsFlyingMonster(monster->classname) ? adjustmentFactor : 1.0f);
		if (random_weight <= cumulative_weight) {
	...
	return chosen_monster ? chosen_monster->classname : nullptr;
}
...
static void PrecacheItemsAndBosses() noexcept {
	...
	// Precachar jefes de todas las categorías
	for (size_t i = 0; i < BOSS_SMALL_SIZE; ++i) {
		PrecacheItem(FindItemByClassname(BOSS_SMALL[i].classname));
	}
	for (size_t i = 0; i < BOSS_MEDIUM_SIZE; ++i) {
		PrecacheItem(FindItemByClassname(BOSS_MEDIUM[i].classname));
	}
	for (size_t i = 0; i < BOSS_LARGE_SIZE; ++i) {
		PrecacheItem(FindItemByClassname(BOSS_LARGE[i].classname));
	}
}
...
static bool Horde_AllMonstersDead() {
	...
	// Añadir un mensaje de depuración para confirmar que todos los monstruos están muertos
...
bool CheckRemainingMonstersCondition(const MapSize& mapSize, WaveEndReason& reason) {
	...
	// Si aún hay monstruos por spawnear o no hay monstruos en total, no hacer nada
...
void Horde_RunFrame() {
	...
	// Obtener el número de jugadores humanos y espectadores una sola vez
	...
	// Cachear el nivel actual para evitar accesos repetidos
		...
		if (g_horde_local.monster_spawn_time <= level.time) {
			...
			if (g_horde_local.num_to_spawn == 0) {
				if (!next_wave_message_sent) {
					...
					next_wave_message_sent = true; // Prevenir mensajes múltiples
				}

				// Transición al estado 'active_wave'
				g_horde_local.state = horde_state_t::active_wave;
				g_horde_local.conditionTriggered = false; // Resetear condición
				g_horde_local.conditionStartTime = 0_sec;
				g_horde_local.waveEndTime = 0_sec;
				g_horde_local.lastPrintTime = 0_sec;
	...
	case horde_state_t::active_wave: {
		...
		// Opcional: spawnear monstruos adicionales durante la ola activa
