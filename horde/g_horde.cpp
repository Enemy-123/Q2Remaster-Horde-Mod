...
namespace HordeConstants {
    ...
    // --- Wave Timing ---
    constexpr gtime_t BASE_MAX_WAVE_TIME = 115_sec;
    constexpr gtime_t TIME_INCREASE_PER_LEVEL = 2.5_sec;
    constexpr gtime_t BOSS_TIME_BONUS = 80_sec;
    constexpr int MONSTERS_FOR_AGGRESSIVE_REDUCTION = 6;
}// namespace HordeConstants

// --- Forward Declarations ---
bool EmergencySpawnMonster(const int32_t levelNum, horde::MonsterTypeID typeId, bool is_additional_monster);
...
struct MonsterCache {
    ...
    void addMonster(horde::MonsterTypeID typeId, float weight) {
        if (count >= MONSTER_CACHE_SIZE || weight <= 0.0f) return;
...
static bool ShouldAttemptHigherLevelSpawn(int32_t currentLevel, bool isRetaliationActive, bool isRecoveryModeActive) {
    ...
    if (currentLevel <= 10) return frandom() < 0.16f; // HordeConstants::CHANCE_SPAWN_HIGHER_EARLY
    if (currentLevel <= 15) return frandom() < 0.05f; // HordeConstants::CHANCE_SPAWN_HIGHER_MID
...
static int32_t CalculateEffectiveMonsterLevel(int32_t currentActualLevel, bool attemptHigherLevel, MonsterWaveType waveTypeForFiltering) {
    ...
    else if (currentActualLevel <= 10) levelBoost = irandom(2, 3);
    else levelBoost = irandom(1, 2);

    int32_t maxLevelCap = currentActualLevel < 7 ? irandom(5, 7) :
                         (currentActualLevel < 15 ? currentActualLevel + 5 : currentActualLevel + 3);
...
static float ApplySpecialModifiers(float weight, const MonsterTypeInfo& monster, const MonsterSelectionContext& ctx) {
    ...
    if (g_insane->integer || g_chaotic->integer) {
        const float difficultyScale = ctx.currentActualLevel <= 10 ? 1.1f :
            ctx.currentActualLevel <= 20 ? 1.2f :
            ctx.currentActualLevel <= 30 ? 1.3f : 1.4f; // Could be HordeConstants::DIFFICULTY_SCALES[]
    ...
    if (ctx.effectiveLevel > ctx.currentActualLevel && monster.minWave > ctx.currentActualLevel) {
        if (monster.minWave - ctx.currentActualLevel > 5) { // HordeConstants::EFFECTIVE_LEVEL_PENALTY_THRESHOLD
...
static horde::MonsterTypeID G_HordePickMonsterType(
    ...
    if (ctx.isRecoveryModeActive) {
        ctx.waveTypeForFiltering = HasWaveType(originalWaveTypeBeforeRecovery_param, MonsterWaveType::Flying) ?
                                   MonsterWaveType::Flying : MonsterWaveType::Ground;
    } else {
...
void InitializeMonsterInfoLUT() {
    if (g_monster_info_lut_initialized) return;
    g_monster_info_lut.fill(nullptr);
    for (const auto& monster_entry : monsterTypes) { // Iterate over the actual monsterTypes array
    ...
    if (developer->integer) gi.Com_PrintFmt("Monster Info LUT Initialized.\n");
}
...
static bool CheckRemainingMonstersCondition(const horde::MapSize& mapSize, WaveEndReason& reason) {
    ...
    // This check determines if the wave *actually ends* due to the independent timer.
    // GetWaveTimer() will reflect this shorter time if it's active.
    ...
    if (safeTotal < 1) safeTotal = 1; // Prevent division by zero
    const float percentageRemaining = (safeTotal > 0) ? (static_cast<float>(remainingMonsters) / safeTotal) : 1.0f;


    // Initialize end time if needed (based on independent timer initially if no condition triggered yet)
    if (g_horde_local.waveEndTime == 0_sec && !g_horde_local.conditionTriggered) {
    ...
    // This happens even if conditions are already triggered, and can shorten g_horde_local.waveEndTime
    if (g_horde_local.conditionTriggered && remainingMonsters <= HordeConstants::MONSTERS_FOR_AGGRESSIVE_REDUCTION) {
        // (Aggressive time calculation logic as before...)
		float base_time = 6.0f + (remainingMonsters * 1.5f); 
        ...
        if (original_remaining_conditional > 0_sec && aggressive_time < original_remaining_conditional) {
            g_horde_local.waveEndTime = currentTime + aggressive_time; // This shortens the conditional timer
            if (developer->integer) {
                gi.Com_PrintFmt("Aggressive time reduction: {:.1f}s remaining (Map Size Mult: {:.2f}) for {} monsters (wave: {}, boss: {})\n",
                    aggressive_time.seconds(), map_size_multiplier, remainingMonsters, current_wave_level,
    ...
    // Determine if warnings should be active:
    // - EITHER a monster-related condition has triggered a timer (g_horde_local.conditionTriggered)
    // - OR the wave is fully deployed and active (g_horde_local.state == active_wave)
    //   (this covers cases where only the independent timer is effectively running because monster counts are still high)
    bool should_issue_warnings = g_horde_local.conditionTriggered ||
                                 (g_horde_local.state == horde_state_t::active_wave && next_wave_message_sent);

    if (should_issue_warnings) {
        const gtime_t actualRelevantRemainingTime = GetWaveTimer(); // Get the SHORTEST time remaining

        if (actualRelevantRemainingTime > 0_sec) { // Only issue warnings if time is actually left
    ...
    // Check if conditional time has expired (this is one way the wave can end)
    // Note: The independent timer check is already done above and will cause `reason = WaveEndReason::TimeLimitReached`
    if (g_horde_local.conditionTriggered && currentTime >= g_horde_local.waveEndTime) {
        reason = WaveEndReason::MonstersRemaining; // Or TimeLimitReached if waveEndTime was from independent
        return true;
    }
    ...
    if (current_wave_level >= 15 && remainingMonsters <= 3 && g_horde_local.conditionTriggered) {
        ...
        if (g_horde_local.conditionTimeThreshold > 0_sec && // Ensure threshold is valid
            elapsed >= (g_horde_local.conditionTimeThreshold * 0.7f)) {
...
bool CheckAndTeleportStuckMonster(edict_t* self) {
	...
	if (!self || !self->inuse || self->deadflag || self->monsterinfo.IS_BOSS || !g_horde || !g_horde->integer) return false;
	...
	bool is_critical_teleport_reason = false; // --- ADDED ---
    ...
    if (self->waterlevel > 0) {
        ...
        if (!can_see_target) {
            ...
            if (developer->integer > 1) gi.Com_PrintFmt("CATS: {} drowning, flagging for immediate CRITICAL teleport.\n", self->classname);
        } else {
	...
	if (!attempt_teleport_now) {
        ...
        if (self->monsterinfo.was_stuck && self->monsterinfo.stuck_check_time > 0_sec) {
            ...
            if(attempt_immediate_teleport_spawn) {
                 if (developer->integer > 1) gi.Com_PrintFmt("CATS: {} potentially stuck on spawn. Flagging for immediate teleport check.\n", self->classname);
	...
	if (!is_critical_teleport_reason && frandom() < 0.4f) {
		if (developer->integer > 2) gi.Com_PrintFmt("CATS: Randomly skipped non-critical teleport attempt for {}.\n", self->classname);
	...
	if (final_dest_origin == vec3_origin) {
	...
	if (final_dest_origin != vec3_origin) {
		if (!IsPositionTooCloseToRecentTeleport(final_dest_origin)) {
            ...
            vec3_t validated_offset_dest = offset_candidate_dest;
			...
			if (IsValidSpawnLocation(validated_offset_dest, self->mins, self->maxs, IsFlying(horde::MonsterTypeRegistry::GetTypeID(self->classname)))) {
				if (!IsPositionTooCloseToRecentTeleport(validated_offset_dest)) {
			...
			teleported = Horde_TeleportMonster(self, potential_dest, final_dest_angles, true, false);

			if (teleported) {
				MarkPositionAsRecentlyTeleported(potential_dest);
				if (used_spawn_point && !use_offset_dest) {
...
int SpawnRetaliationAmbush(const horde::MapSize& mapSize, int32_t waveLevel, edict_t* target_player) {
	...
	for (int i = 0; i < ambushSize; ++i) {
		...
		if (!monster) { // Fallback to general emergency spawn
			// --- MODIFIED SECTION for Fallback ---
			// Call EmergencySpawnMonster with is_additional_monster = true
			if (EmergencySpawnMonster(waveLevel, monster_typeId, true)) { // MODIFIED: true for additional
				spawnedCount++;
				// DO NOT decrement g_horde_local.num_to_spawn or g_horde_local.queued_monsters.
				// g_totalMonstersInWave is handled by EmergencySpawnMonster.
				continue; // Skip champion logic for this emergency fallback as 'monster' ptr is not set
			} else {
			// --- END MODIFIED SECTION for Fallback ---
				if (developer->integer > 1) {
					gi.Com_PrintFmt("SpawnRetaliationAmbush: Failed to spawn monster type {} (fallback).\n", (int)monster_typeId);
				}
				continue;
			}
		}

		// This block only executes if the targeted spawn near player succeeded
		if (monster && monster->inuse) {
			spawnedCount++;
			// --- MODIFIED SECTION for Targeted Spawn ---
			// DO NOT decrement g_horde_local.num_to_spawn or g_horde_local.queued_monsters.
			// This is an additional monster, so increment g_totalMonstersInWave.
			...
			// --- END MODIFIED SECTION for Targeted Spawn ---
	...
	// Note: The separate logic that adds to g_horde_local.queued_monsters for retaliation
	// is a distinct mechanism to reinforce the *main* wave and is handled elsewhere.
	return spawnedCount;
}
...
// --- REVISED EmergencySpawnMonster ---
// Now takes is_additional_monster to control g_totalMonstersInWave increment
bool EmergencySpawnMonster(const int32_t levelNum, horde::MonsterTypeID typeId, bool is_additional_monster) { // MODIFIED: Added is_additional_monster
	...
	if (is_additional_monster) {
		if (g_totalMonstersInWave < std::numeric_limits<uint16_t>::max()) {
			...
			gi.Com_PrintFmt("Warning: g_totalMonstersInWave overflow prevented in EmergencySpawnMonster (additional).\n");
		}
	}
	// --- END MODIFIED SECTION ---

	if (developer->integer) {
		gi.Com_PrintFmt("EMERGENCY SPAWN SUCCESSFUL: Spawned {} at emergency position (Additional: {})\n",
...
// Updated SpawnAmbushMonsters to use the refactored G_HordePickMonsterType
...
int SpawnAmbushMonsters(const horde::MapSize& mapSize, int32_t waveLevel) {
    ...
    const bool isRetaliation_ctx = g_horde_retaliation_active;
    const bool isRecovery_ctx = false;
    ...
    if (monster_typeId_for_ambush == horde::MonsterTypeID::UNKNOWN) {
        if (developer->integer > 1) {
            gi.Com_PrintFmt("SpawnAmbushMonsters: G_HordePickMonsterType returned UNKNOWN. Using hardcoded fallback.\n");
        }
        if (waveLevel >= 15) {
    ...
    for (int i = 0; i < ambushSize && consecutive_spawn_failures_ambush < HordeConstants::MAX_CONSECUTIVE_FAILURES_BEFORE_EMERGENCY; /* i incremented on success */) {
        if (safety_iteration_counter++ > MAX_AMBUSH_ITERATIONS) {
            if (developer->integer) gi.Com_PrintFmt("WARNING: Ambush spawn safety iteration limit reached.\n");
            break;
        }
        if (consecutive_spawn_failures_ambush > 0 && level.time < last_failed_ambush_spawn_time + AMBUSH_SPAWN_RETRY_DELAY) {
             // Optionally continue or yield, for now just proceed to check type
        }

        if (monster_typeId_for_ambush == horde::MonsterTypeID::UNKNOWN) {
            if (developer->integer) gi.Com_PrintFmt("SpawnAmbushMonsters: CRITICAL - monster_typeId_for_ambush is UNKNOWN before spawning loop.\n");
            break;
        }

        // --- MODIFIED SECTION ---
        // Call EmergencySpawnMonster with is_additional_monster = true
        // These monsters are additional and should increment g_totalMonstersInWave.
        // They should NOT be drawn from g_horde_local.num_to_spawn or g_horde_local.queued_monsters.
        if (EmergencySpawnMonster(waveLevel, monster_typeId_for_ambush, true)) { // MODIFIED: true for additional
            ambushSuccessCount++;
            consecutive_spawn_failures_ambush = 0;
            // DO NOT decrement g_horde_local.num_to_spawn or g_horde_local.queued_monsters here.
            i++;
        } else {
        // --- END MODIFIED SECTION ---
...
static int ExecuteEmergencySpawnProcedure(int32_t spawnable_this_call, int32_t currentLevel);
...
void SpawnMonsters() {
    ...
    const gtime_t current_time = level.time; // Though current_time is not directly used in this version, it's good practice if helpers need it
    const horde::MapSize& mapSize = g_horde_local.current_map_size;
    ...
    RebuildSpawnPointCacheIfNeeded(); // This will set g_spawn_points_cached and reset need_spawn_cache_reset if it was true

    // --- MODIFIED SECTION for the fix ---
    if (g_potential_spawn_points.empty()) {
        if (developer->integer) gi.Com_PrintFmt("SpawnMonsters: No potential spawn points found in cache.\n");
        
        // Increment failure count, but don't let it exceed emergency threshold based on this alone yet,
        // as we want to try rebuilding the cache first if it wasn't explicitly requested.
        // We make sure it doesn't immediately trigger emergency if this is the *only* failure source for a few frames.
        if (g_consecutive_spawn_failures < HordeConstants::MAX_CONSECUTIVE_FAILURES_BEFORE_EMERGENCY -1 ) { 
             g_consecutive_spawn_failures++;
        }

        // If the cache was supposedly up-to-date (need_spawn_cache_reset was false before Rebuild...)
        // but it's still empty, it means the last rebuild yielded no points.
        // Force a new rebuild attempt on the next frame.
        // This handles cases where all points might have become invalid due to game state
        // (e.g., all blocked by players/monsters temporarily).
        if (!need_spawn_cache_reset) { // Check if a reset wasn't already pending for next frame from other sources
            need_spawn_cache_reset = true; // Force a full cache rebuild attempt next call
            if (developer->integer) gi.Com_PrintFmt("SpawnMonsters: Forcing spawn point cache reset for next frame due to empty cache.\n");
        }
        // It's also possible that g_spawn_points_cached is false if Rebuild... failed to find any points AT ALL on the map.
        // In that severe case, repeatedly trying to rebuild might not help, but it's a safe fallback.
        return;
    }
    // --- END MODIFIED SECTION ---
    ...
    DetermineSpawnStrategy(
        ...
        g_recovery_mode_active, // Pass by value, modification happens inside or globally
    ...
    if (use_emergency_spawn_flag) {
        ...
        if (num_spawned_this_call > 0 && g_recovery_mode_active) {
             if (developer->integer) gi.Com_PrintFmt("SpawnMonsters: Exiting recovery mode due to successful EMERGENCY spawns.\n");
             g_recovery_mode_active = false;
        ...
        if (g_recovery_mode_active && g_consecutive_spawn_failures == 0 && num_spawned_this_call > 0) {
            if (developer->integer) gi.Com_PrintFmt("SpawnMonsters: Exiting recovery mode due to successful NORMAL spawns.\n");
            g_recovery_mode_active = false;
            current_wave_type = g_original_wave_type_before_recovery;
            g_original_wave_type_before_recovery = MonsterWaveType::None;
        }
...
static int ExecuteEmergencySpawnProcedure(int32_t spawnable_this_call, int32_t currentLevel) { // MODIFIED: Removed last_spawned_this_call parameter
    int emergency_spawned_count = 0;
    // MODIFIED: Removed last_spawned_this_call initialization
    ...
    for (int i = 0; i < std::min(spawnable_this_call, 3); ++i) {
        ...
        if (EmergencySpawnMonster(currentLevel, emergency_type, false)) {
...
static void RebuildSpawnPointCacheIfNeeded() {
    if (!g_spawn_points_cached || need_spawn_cache_reset) {
        ...
        g_consecutive_spawn_failures = 0; 
        if (developer->integer) gi.Com_PrintFmt("Spawn Point Cache Rebuilt: {} points ({} flying).\n", g_potential_spawn_points.size(), g_cached_flying_spawn_count);
    }
}
...
static bool ValidateSpawnPointForMonster(edict_t* spawn_point, const SpawnPointData& sp_data, gtime_t current_time, bool recovery_mode_active_param) {
    ...
    for (const auto* const player : active_players_no_spect()) {
        if ((spawn_point->s.origin - player->s.origin).lengthSquared() < HordeConstants::MIN_PLAYER_DIST_SQ_SPAWNPOINT) { // MIN_PLAYER_DIST_SQ_SPAWNPOINT was used before, check which is correct
            IncreaseSpawnAttempts(spawn_point);
            g_consecutive_spawn_failures++;
            return false;
        }
...
static edict_t* FindValidSpotAndSpawn(edict_t* spawn_point, horde::MonsterTypeID monster_type, int32_t currentLevel, float champion_chance) { // MODIFIED: Return type edict_t*, removed out_monster
    vec3_t final_origin;
    vec3_t final_angles;
    ...
    if (found_valid_spot) {
        ...
        if (monster) {
            ...
            // CORRECTED CHAMPION CHECK:
            if (monster_still_valid && currentLevel >= 3 && !champion_spawned_this_wave && 
                champion_spawn_cooldown_ends_at < level.time && // Use the time-based cooldown
                ...
                if (!monster->inuse) { monster_still_valid = false; }
                    ...
                    champion_spawned_this_wave = true; 
                    // Set the new time-based cooldown:
                    champion_spawn_cooldown_ends_at = level.time + random_time(10_sec, 20_sec); // Example range
            ...
            if (monster_still_valid) {
                // MODIFIED: Removed out_monster assignment
                if (used_alternative) ApplySuccessfulAlternativeCooldown(spawn_point);
                else OnSuccessfulSpawn(spawn_point);
                return monster; // MODIFIED: Return the spawned monster
            } else {
                if (developer->integer > 1) gi.Com_PrintFmt("SpawnMonsters: Monster {} invalid after bonuses.\n", monster->classname ? monster->classname : "Unknown");
                g_consecutive_spawn_failures++;
                // If monster became invalid, it should be freed or handled by the caller.
                // Since SpawnMonsterByTypeID might return a non-inuse edict that gets freed later,
                // returning nullptr here is safer if it's truly unusable.
                if(monster->inuse) G_FreeEdict(monster); // Free it if it became invalid but is still inuse
                return nullptr; // MODIFIED: Return nullptr on failure
            }
        } else {
...
void Horde_RunFrame() {
	...
	static int32_t last_wave_level_for_failsafe = 0; 
	...
	case horde_state_t::spawning: {
		...
		if (spawning_state_start_time == 0_sec) {
			...
			g_horde_local.state = horde_state_t::active_wave; 
			spawning_state_start_time = 0_sec; 
			break; 
		}
		...
		if (g_horde_local.monster_spawn_time <= currentTime) {
			...
			if (!IsBossWave() || boss_spawned_for_wave) {
				SpawnMonsters(); // MODIFIED: Return value is void, no longer captured
			}
...
bool GetPredictedScaledBounds(horde::MonsterTypeID typeId, vec3_t& out_mins, vec3_t& out_maxs) {
    if (!g_monster_info_lut_initialized) { // Should have been called in Horde_Init
        InitializeMonsterInfoLUT();
        if (!g_monster_info_lut_initialized && developer->integer) { // Still not initialized?
    ...
    if (info) {
        ...
        if (developer->integer) gi.Com_PrintFmt("GetPredictedScaledBounds: WARN - MonsterTypeInfo not found or invalid TypeID {}, using generic bounds.\n", static_cast<int>(typeId));
        return false; 
    }
    ...
    if (scale <= 0.0f) { // Check for invalid scale
        if (developer->integer) gi.Com_PrintFmt("GetPredictedScaledBounds: WARN - MonsterTypeID {} has invalid scale %.2f. Using unscaled default bounds.\n", static_cast<int>(typeId), scale);
