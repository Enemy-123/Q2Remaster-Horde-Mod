...
DIE(doppleganger_die) (edict_t* self, edict_t* inflictor, edict_t* attacker, int damage, const vec3_t& point, const mod_t& mod) -> void
{
	...
	// Verifica si self->enemy no está asignado y busca un enemigo adecuado
	if (!self->enemy)
	{
		for (size_t i = 0; i < globals.max_edicts; i++)
		{
			...
			if (g_edicts[i].svflags & SVF_MONSTER)
			{
				if (!g_edicts[i].deadflag && g_edicts[i].health > 0)
				{
					...
					break;  // Asigna el primer enemigo encontrado
				}
			}
		}
	}

	// Verifica si self->enemy está asignado y es un monstruo
