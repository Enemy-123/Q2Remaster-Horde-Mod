...
FIELD_AUTO(monsterinfo.retarget_scan_time),
...
FIELD_AUTO(monsterinfo.failsafe_teleport_count),
FIELD_AUTO(monsterinfo.isfriendlyspawn),
FIELD_AUTO(monsterinfo.issummoned),
FIELD_AUTO(monsterinfo.upkeep_enabled),
FIELD_AUTO(monsterinfo.IS_BOSS),
FIELD_AUTO(monsterinfo.effects_applied),
FIELD_AUTO(monsterinfo.BOSS_DEATH_HANDLED),
FIELD_AUTO(monsterinfo.boss_loot_share_count),
FIELD_AUTO(monsterinfo.boss_loot_share_index),
...
void read_save_type_json(const Json::Value& json, void* data, const save_type_t* type, const char* field)
{
	...
	case ST_FIXED_STRING:
		...
		{
			size_t len = strlen(json.asCString());
			if (type->count && len >= type->count)
				json_print_error(field, "fixed length string overrun", false);
			else
			{
				memcpy((char*)data, json.asCString(), len);
				((char*)data)[len] = 0;
			}
		}
		else if (json.isArray())
